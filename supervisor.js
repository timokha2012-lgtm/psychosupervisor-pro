...
const Supervisor = {
  ...
  analyzeTranscript(text, context) {
    ...
    // Если формат не распознан — анализируем как свободный текст
    if (!psychLines.length) {
      // Попробуем разбить по строкам и чередовать спикеров
      const allLines = text.split('\n').filter(l => l.trim().length > 10);
      allLines.forEach((line, i) => {
        const cleaned = line.replace(/^(\d{2}:\d{2}:\d{2}\s+)?(Speaker\s*\d+:?\s*|\[Speaker\s*\d+\]\s*)/i, '').trim();
        if (cleaned) {
          if (i % 2 === 0) psychLines.push(cleaned);
          else clientLines.push(cleaned);
        }
      });
      if (!psychLines.length) {
        // Последний вариант — весь текст как один блок
        psychLines.push(text.substring(0, 500));
      }
    }
